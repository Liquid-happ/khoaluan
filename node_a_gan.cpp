...
uint8_t macNode3[] = {0xD4, 0xE9, 0xF4, 0xA4, 0xF2, 0xB8}; // Đăng ký Node 3
...
void loop() {
  ...
  if (millis() - last_read_time >= 2000) {
      ...
      float VOLTAGE_DIVIDER_RATIO = 3.0; // Hệ số cho trở 200k và 100k
      float CALIBRATION_FACTOR = 1.0;    // Đổi số này nếu bị lệch Volt (vd: 1.05 hoặc 0.95)
      
      // Tính điện áp thực tế (Không ép cứng để dễ kiểm tra)
      float bat_vol = (adc_val / 4095.0) * 3.3 * VOLTAGE_DIVIDER_RATIO * CALIBRATION_FACTOR; 

      // Ép cứng % hiển thị từ 0 - 100%
